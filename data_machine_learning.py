...
param_grid = {'C': np.arange(0.1, 10, 0.5), 'gamma': np.linspace(0.01, 1, 10)}

grid_search = GridSearchCV(svm_model, param_grid, cv=5)
grid_search.fit(X_train_scaled, y_train_select_1d)

print("최적: ", grid_search.best_params_)


# In[55]:


svm_model = SVC(kernel='rbf', C = 2.6 , gamma = 0.34)
svm_model.fit(X_train_scaled, y_train_select_1d)
...
param_grid_r = {'C': np.arange(0.1, 10, 0.5), 'gamma': np.linspace(0.01, 1, 10)}

grid_search_r = GridSearchCV(svm_model_r, param_grid_r, cv=5)
grid_search_r.fit(X_train_scaled_r, y_train_select_1d_r)

print("최적: ", grid_search_r.best_params_)
